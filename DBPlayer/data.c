...
void DeleteData()
{
  ...
  if (data)
  {
    FSX_FreeFullPath(data->file.fullpath);
    mfree(data);
...
void SaveData(bool save, int item)
{
  ...
  if (save)
  {
    int f = _fopen( FILE_PATH, FILE_NAME, FSX_O_WRONLY, FSX_S_IWRITE|FSX_S_IREAD, NULL );
    if(f >= NULL) 
    {
      fwrite(f, &data->setting, sizeof(SETTING_DATA));
      fclose(f);
    }
  }
}
...
{
  int f = _fopen( FILE_PATH, FILE_NAME, FSX_O_RDONLY, FSX_S_IREAD|FSX_S_IWRITE, NULL );
  if(f >= NULL) 
  {
    fread(f, &Data->setting, sizeof(SETTING_DATA));
    fclose(f);
  }
}
