...
{
  if (disp_obj->buf_text != EMPTY_TEXTID) TextID_Destroy(disp_obj->buf_text);
}

void Time_OnRedraw(DISP_OBJ_TIME* disp_obj, int, int, int)
{
  DrawRect(55, 130, 185, 190, clBlack, clBlack);
  ...
  if (!disp_obj->curs) DrawRect(60, 186, 115, 188, clWhite, clWhite);
  else DrawRect(125, 186, 180, 188, clWhite, clWhite);
}
...
void Time_OnKey(DISP_OBJ_TIME* disp_obj, int key, int, int repeat, int mode)
{
  if (mode == KBD_SHORT_RELEASE || mode == KBD_REPEAT)
  {
    ...
    else if (mode == KBD_REPEAT && repeat > 5) disp_obj->cstep = 3;
    ...
    case KEY_UP:
      if (!disp_obj->curs)
      {
        if ((disp_obj->buf_min += disp_obj->cstep) >= disp_obj->track_time_min)
        {
          ...
          if (disp_obj->buf_sec > disp_obj->track_time_sec) disp_obj->buf_sec = disp_obj->track_time_sec;
        }
      }
      else
      {
        ...
        {
          if ((disp_obj->buf_sec += disp_obj->cstep) >= disp_obj->track_time_sec) disp_obj->buf_sec = disp_obj->track_time_sec;
        }
        else 
        {
    ...
    case KEY_DOWN:
      if (!disp_obj->curs)
      {
        if ((disp_obj->buf_min -= disp_obj->cstep) < 0) disp_obj->buf_min = 0;
      }
      else
      {
        if ((disp_obj->buf_sec -= disp_obj->cstep) < 0)
        {
          ...
          else disp_obj->buf_sec = 0;
        }
      }
      break;
      
    case KEY_LEFT:
      if (!disp_obj->curs) disp_obj->curs = TIME_SEC;
      else disp_obj->curs = TIME_MIN;
      break;
...
{
  GUI* gui_time = (GUI*)malloc(sizeof(GUI));
  if(!GUIObject_Create(gui_time, Time_destroy, Time_create, book, NULL, UIDisplay_Main, sizeof(GUI))) mfree(gui_time);
  if (book) BookObj_AddGUIObject(book, gui_time);
  
  GUIObject_SetStyle(gui_time, UI_OverlayStyle_TrueFullScreen);
  GUIObject_SetTitleType(gui_time, UI_TitleMode_None);
  
  GUIObject_SoftKeys_SetAction(gui_time, ACTION_SELECT1, Time_OnSelect);
  GUIObject_SoftKeys_SetText(gui_time, ACTION_SELECT1, TEXT_TIME);
  GUIObject_SoftKeys_SetAction(gui_time, ACTION_BACK, OnBackTime);
  GUIObject_SoftKeys_RemoveBackground(gui_time);
  GUIObject_Show(gui_time);
  
  DBP_DATA* data = GetData();
  data->gui_time = gui_time;
}
