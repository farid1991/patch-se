...
void Free_SkinData(SKIN_DATA *Skin)
{
  if (Skin)
  {
    ...
    Skin->icon_playerstate_enable = NULL;
    Skin->icon_playerstate_x = NULL;
    Skin->icon_playerstate_y = NULL;
...
void SaveCurrentSkinPath(wchar_t *skin_path)
{
    ...
    if (filesize)
    {
      ...
      if (file >= 0)
      {
        ...
        sscanf(buffer, format,
               ...
               &Skin->icon_playerstate_enable, &Skin->icon_playerstate_x, &Skin->icon_playerstate_y,
...
void ReloadSkinData(ADVPLAYER_DATA *data, wchar_t *new_skin_path)
{
  ...
  {
    DispObject_SetStyle(data->MediaPlayer_Audio, UI_OverlayStyle_FullScreenNoStatus);
  ...
  else
  {
    ...
    DispObject_WindowSetSize(data->MediaPlayer_NowPlaying, Display_GetWidth(UIDisplay_Main), Display_GetHeight(UIDisplay_Main) - SOFTKEYS_H);
  }
  InvalidateDispObj(data->MediaPlayer_Audio);
  InvalidateDispObj(data->MediaPlayer_NowPlaying);
}

void GetCoverArt(ADVPLAYER_DATA *data)
{
  if (data->HasCover)
    IMAGE_FREE(data->CoverArt);

  data->CoverSize = MetaData_ExtractCover(data->CurrentTrack->path, data->CurrentTrack->name);
  if (data->CoverSize)
  {
