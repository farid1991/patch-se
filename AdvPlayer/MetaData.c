...
int Songname2Albumname(wchar_t *albumname, wchar_t *songname, int len)
{
  if (!len)
    return 0;
  int i = 0;
  ...
  while (i < len - extlen)
  {
    albumname[i] = songname[i];
    i++;
  }
  wstrcat(albumname, L"png");
  return 1;
}
...
int MetaData_ExtractCover(wchar_t *path, wchar_t *name)
{
  ...
  if (FSX_IsFileExists(SKIN_CFG_PATH, L"albumart.png"))
  {
    int error;
