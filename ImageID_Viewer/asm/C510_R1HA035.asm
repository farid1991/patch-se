...
//------------------------------------------------------------------------------
        ...
        defadr onBackPressed,0x1554EDF0+1
        defadr onLongBackPressed,0x1554EE10+1
        
        defadr page_ServiceMenu_ViewTextLabels,0x15A71E98
        
        defadr pg_ServiceMenu_Test_Security_ExitAction,0x15072180+1
        defadr pg_ServiceMenu_TestMenu_ReturnStandbyAction,0x1554EDE4+1

	EXTERN New_SetTexts
        EXTERN New_SetAction
...
Patch_SetAction:
	LDR	R0, [R0,#0x20]
        LDR	R3, =ListMenu_GetSelectedItem
        BLX	R3
        CMP	R0, #4
        BEQ	j_newitem_link
        LDR	R1, =page_ServiceMenu_ViewTextLabels
        LDR     R3, =0x14DF8E20+1
        BX      R3
...
//--------------------------------------
        ...
        RSEG	PATCH_SET_TEXTS
        CODE16
	LDR	R3, =Patch_SetTexts
	BX	R3
        
        RSEG	PATCH_SET_ACTION
        CODE16
        LDR	R3, =Patch_SetAction
        BX	R3
        
        RSEG    PATCH_FIX_SETCURSOR
        CODE16
        CMP     R0, #5
#endif
        END
