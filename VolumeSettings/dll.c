...
void dll_DrawString(int font, TEXTID text_id, int align, int x1, int y1, int x2, int y2, int text_color)
{
  ...
  IUIRichTextLayoutOptions * pIRichTextLayoutOptions = NULL;
  ...
  pTextRenderingFactory->CreateRichTextLayoutOptions(&pIRichTextLayoutOptions);
  pTextRenderingFactory->CreateRichTextLayout(pRichText, NULL, pIRichTextLayoutOptions, &pRichTextLayout);
  ...
  pIRichTextLayoutOptions->SetLineBreakModel(UILineBreakBit_OK_To_Break_On_Glyph);
  ...
  if (pIRichTextLayoutOptions)
    pIRichTextLayoutOptions->Release();
  if (pRichTextLayout)
