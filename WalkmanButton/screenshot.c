...
BMP_HEADER *CreateHeader(int width, int height)
{
  ...
  hdr->ID = MAGIC_WORD;
  hdr->size = width * height * 3 + SIZE_WO_MAGIC;
  hdr->dataoffset = DATA_OFFSET;
  hdr->sizeofheader = HEADER_SIZE;
  hdr->width = width;
  hdr->height = height;
  hdr->planes = NBR_OF_PLANE;
  hdr->bpp = BITSPERPLANE;
  hdr->sizeofimage = width * height * 3;
  return hdr;
}
...
void Screenshoter(int xsize, int ysize)
{
  ...
  wchar_t *path = L"/tpa/user/picture/SnapShots";
  wchar_t *fname = WStringAlloc(50);
