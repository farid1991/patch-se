...
{
  int pos = ListMenu_GetSelectedItem(FMSettings_Book->sub_menu);
  ListMenu_DestroyItems(FMSettings_Book->sub_menu);
  ListMenu_SetItemCount(FMSettings_Book->sub_menu, count);
  ListMenu_SetCursorToItem(FMSettings_Book->sub_menu, pos);
  GUIObject_SoftKeys_SetVisible(FMSettings_Book->sub_menu, ACTION_SELECT1, TRUE);
...
void SetActivate(BOOK *book, int count)
{
  ...
  int item = ListMenu_GetSelectedItem(FMSettings_Book->sub_menu);
  ...
  else if (item == 2)
  {
    if (data->temp.round)
    {
      if (FMSettings_Book->element == ITEM_FREQ_INDICATOR)
  ...
  ListMenu_SetItemSecondLineText(FMSettings_Book->sub_menu, item, text);
  FMSettings_Book->changed = TRUE;
}
...
void Show_OnSelect(BOOK *book, GUI *gui)
{
  ...
  SETTING_BOOK *FMSettings_Book = (SETTING_BOOK *)book;
  data->temp.state = OneOfMany_GetSelected(FMSettings_Book->option_menu);
  FREE_GUI(FMSettings_Book->option_menu);
...
void Align_OnSelect(BOOK *book, GUI *gui)
{
  SETTING_BOOK *FMSettings_Book = (SETTING_BOOK *)book;
  FmRadio_Data *data = GetData();
  data->temp.align = OneOfMany_GetSelected(FMSettings_Book->option_menu);
  TEXTID text = EMPTY_TEXTID;
  ...
  ListMenu_SetItemSecondLineText(FMSettings_Book->sub_menu, SUBITEM_TXT_ALIGN, text);
  FREE_GUI(FMSettings_Book->option_menu);
...
void OneOfMany_SetMode(BOOK *book, int type)
{
  ...
  FmRadio_Data *data = GetData();
  FMSettings_Book->option_menu = CreateOneOfMany(FMSettings_Book);

  if (type == OOM_STATE)
  {
    GUIObject_SetTitleText(FMSettings_Book->option_menu, TextID_Global(ID_SHOW));

    TEXTID texts[2];
    texts[0] = TEXT_OFF;
    texts[1] = TEXT_ON;
    OneOfMany_SetTexts(FMSettings_Book->option_menu, texts, MAXELEMS(texts));
    OneOfMany_SetChecked(FMSettings_Book->option_menu, data->temp.state);
    GUIObject_SoftKeys_SetAction(FMSettings_Book->option_menu, ACTION_SELECT1, Show_OnSelect);
  }
  else if (type == OOM_ALIGN)
  {
    GUIObject_SetTitleText(FMSettings_Book->option_menu, TextID_Global(ID_ALIGN));
    ...
    texts[2] = TEXT_CENTRE;
    OneOfMany_SetTexts(FMSettings_Book->option_menu, texts, MAXELEMS(texts));
    OneOfMany_SetChecked(FMSettings_Book->option_menu, data->temp.align);
    GUIObject_SoftKeys_SetAction(FMSettings_Book->option_menu, ACTION_SELECT1, Align_OnSelect);
  }

  GUIObject_SoftKeys_SetAction(FMSettings_Book->option_menu, ACTION_BACK, OnBack);
  GUIObject_Show(FMSettings_Book->option_menu);
}
...
void Color_OnRedraw(DISP_OBJ_COLOR *disp_obj, int, int, int)
{
  ...
  for (int i = 0; i != 4; i++)
  {
    ...
    if (disp_obj->current_column == i)
      DrawRect(x1 + start_column - 2, y1 + fsize - 2 + 50, x1 + start_column + column_width + 2, y1 + fsize + column_height + 2, clBlack, clWhite); // �������
...
{
  SETTING_BOOK *FMSettings_Book = (SETTING_BOOK *)book;
  DISP_OBJ_COLOR *disp_obj = (DISP_OBJ_COLOR *)GUIObject_GetDispObject(FMSettings_Book->color_picker);
  ...
  ListMenu_SetItemSecondLineText(FMSettings_Book->sub_menu, item, TextID_Create(data->buf, ENC_UCS2, TEXTID_ANY_LEN));
  FREE_GUI(FMSettings_Book->color_picker);
  FMSettings_Book->changed = TRUE;
}
...
void Create_ColorPicker(BOOK *book, int type)
{
  ...
  if (!GUIObject_Create(gui_color, Color_destruct, Color_contruct, FMSettings_Book, Color_CallBack, UIDisplay_Main, sizeof(GUI)))
    mfree(gui_color);
  ...
  {
    unsigned int color = data->temp.color2;
    disp_obj->r = COLOR_GET_R(color);
    disp_obj->g = COLOR_GET_G(color);
    disp_obj->b = COLOR_GET_B(color);
    disp_obj->a = COLOR_GET_A(color);
  ...
  if (FMSettings_Book)
    BookObj_AddGUIObject(FMSettings_Book, FMSettings_Book->color_picker);

  GUIObject_SetTitleText(FMSettings_Book->color_picker, TEXT_COLOR);
  GUIObject_SetTitleType(FMSettings_Book->color_picker, UI_TitleMode_Small);
  GUIObject_SoftKeys_SetAction(FMSettings_Book->color_picker, ACTION_SELECT1, Color_OnSelect);
  GUIObject_SoftKeys_SetAction(FMSettings_Book->color_picker, ACTION_BACK, Color_onBack);
  GUIObject_Show(FMSettings_Book->color_picker);
