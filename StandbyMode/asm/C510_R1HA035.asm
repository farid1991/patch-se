...
a       equ b
	...
	defadr GUIObject_TabTitleRemove,0x143C1DAC+1
        ...
        defadr FlightMode_SetState,0x144D2134+1
...
//////////////////
        ...
        defadr INetworkServices_GetNetworkName,0x14CF10F4+1
        defadr CallInfo_Get,0x1055821C+1

        defadr IsTimerBook,0x15548310+1

        defadr get_sys_uptime,0x140CC684+1
        defadr arm_division,0x143DC1EC

        defadr Show,0x14E73034+1
        defadr Return_Clear,0x14E7302E+1
        defadr Show_Clear,0x14E73060+1

        defadr retSetIcon,0x1417A770+1
        defadr sub_1416A1FC,0x1417A5B4+1
        ...
        EXTERN  Stop_CallStateTimer
        EXTERN  New_SetTrayIcon
        EXTERN  New_StatusIndication_SetItemText

        RSEG    PATCH_STATUSROW_ONREDRAW
        DATA
        DCD     New_StatusRow_onRedraw

        RSEG    PATCH_SOFTKEYS_ONREDRAW
        DATA
        DCD     New_Softkeys_onRedraw

        RSEG    PATCH_STATUSINDICATION_ONREDRAW
        DATA
        DCD     New_StatusIndication_onRedraw

        RSEG    PATCH_STATUSINDICATION_ONCREATE
        DATA
        DCD     New_StatusIndication_onCreate

        RSEG    PATCH_STATUSINDICATION_ONCLOSE
        DATA
        DCD     New_StatusIndication_onClose

        RSEG    PATCH_MAININPUT_ONKEY
        DATA
        DCD     New_MainInput_onKey

        RSEG    PATCH_CALLOFF_CLEAR
        CODE16
        LDR     R0, =CallClear
        BX	R0
...
call_ended:
        BL      Stop_CallStateTimer
        LDR     R3, =Show_Clear
        BX      R3

start_timer:
        BL      Start_CallStateTimer
        LDR     R1, =0xF83C
        MOV     R0, SP
        LDR     R3, =Show
        BX      R3

        RSEG    PATCH_SET_TRAY_ICON
        CODE16
        LDR     R3, =_newSetTrayIcon
        BX      R3
