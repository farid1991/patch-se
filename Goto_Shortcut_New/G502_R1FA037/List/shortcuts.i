#line 1 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\shortcuts.c"
...
#line 13 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\Target\\G502_R1FA037.h"

#line 20 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\Target\\G502_R1FA037.h"
...
#line 94 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\Types.h"




// if( GetChipID( ) &CHIPID_MASK == CHIPID_DB... )
#line 108 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\Types.h"
...
#line 608 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\Types.h"



//fopen/_fopen (mode), FSTAT.st_mode
#line 619 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\Types.h"
...
#line 2 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\shortcuts.c"
...
#line 3 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\shortcuts.c"

#line 1 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\shortcuts.h"
...
typedef struct
{
  wchar_t shortcuts[9][0x1F];
}SHORTCUT;

wchar_t *getShortcutActions(int pos);
int getShortcutsNum();
...
#line 5 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\shortcuts.c"
#line 1 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\struct.h"
...
#line 6 "E:\\MOd\\ELFs\\SVN\\patch\\Patch_Goto_Shortcut_New\\shortcuts.c"

wchar_t *getShortcutActions(int pos)
{
  int f;
  wchar_t *wstr = new wchar_t[0x1F+1];
  if ((f = _fopen(L"/tpa/system/settings",L"goto.sc",0x0001,0x00000100|0x00000080,0)) >= 0)
  {
    SHORTCUT* sc = (SHORTCUT*) malloc(sizeof(SHORTCUT));
    memset(sc,0,sizeof(SHORTCUT));
    fread(f,sc,sizeof(SHORTCUT));
    wstrcpy(wstr,sc->shortcuts[pos]);
    fclose(f);
    mfree(sc);
    return wstr;
  }
...
int getShortcutsNum()
{
  for(int i = 0; i < 9; i++)
  {
    if(wstrlen(getShortcutActions(i)) == 0) return i;
  }
  return 9;
}

int InsertAction(wchar_t *action, int pos)
{
  if (pos > 9) return (-1);
  
  int f;
  if ( (f=_fopen(L"/tpa/system/settings",L"goto.sc", 0x0004|0x0200, 0x00000100|0x00000080, 0)) >=0 )
  {
    SHORTCUT* FData = (SHORTCUT*) malloc(sizeof(SHORTCUT));
    memset(FData,0,sizeof(SHORTCUT) );
    wstrcpy(FData->shortcuts[pos],action);
    fwrite(f,FData,sizeof(SHORTCUT));
    fclose(f);
    mfree(FData);
  }
  return 0;
...
int RemoveAction( int pos )
{
  int f, i;
  if ( (f=_fopen(L"/tpa/system/settings",L"goto.sc", 0x0200|0x0004, 0x00000100|0x00000080, 0)) >=0 )
  {
    SHORTCUT* FData = (SHORTCUT*) malloc(sizeof(SHORTCUT));
    memset(FData,0,sizeof(SHORTCUT) );
    for (i = 0; i < 0x1F ; i++ )
    {
      FData->shortcuts[i][pos] = 0;
    }
    fwrite(f,FData,sizeof(SHORTCUT));
    fclose(f);
    mfree(FData);
...
void onShortcutSet( BOOK* MainMenu, GUI* gui)
{
  MyBOOK* mbk = (MyBOOK*)FindBook(IsMyBook);
  int pos = ListMenu_GetItemCount(mbk->slist) + 1; 
  
  wchar_t* name_buf = MenuBook_Desktop_GetSelectedItemID( MainMenu );

  if (name_buf)
  {
    InsertAction(name_buf, pos);
    mfree( name_buf );
  }
  FreeBook( MainMenu );
...
{
  BookObj_ReturnPage( book, 0x4 );
  return 1;
}

int CreateMainMenu( void* r0, BOOK* book )
{
...
{
  0xE, CreateMainMenu,
  0x3,   onPrevious_MainMenu_DB,
  0x4,     onCancel_MainMenu_DB,
  0,        0
};
PAGE_DESC SelectShortcut_page = { "Goto_SelectShortcut_Page", 0, SelectShortcut_PageEvents };
...
void DelShortcut(BOOK * book , GUI* lt)
{
  ...
  char param[0x1F];
  wstr2strn(param,getShortcutActions(item),0x1F-1);
  TEXTID str_id;
  void * w_buf;

  w_buf = SHORTCUT_DESC_A2_Init(param);



  str_id = Shortcut_Get_MenuItemName( w_buf );
  delete( w_buf );
  if ( str_id == 0x6FFFFFFF ) str_id = TextID_Create( param, ENC_LAT1, 0xFFFF );

  mbk->YesNoQuestion = CreateYesNoQuestion(book,0);
  YesNoQuestion_CreatePreQuestion( mbk->YesNoQuestion, 0x2727 );
  YesNoQuestion_CreateQuestion( mbk->YesNoQuestion, str_id );
  YesNoQuestion_SetIcon(mbk->YesNoQuestion,0xF831);
  GUIObject_SoftKeys_SetAction(mbk->YesNoQuestion,0xFC5,Yes2DelShortcut);
  GUIObject_SoftKeys_SetAction(mbk->YesNoQuestion,0xFBE,Cancel2DelShortcut);
  GUIObject_SoftKeys_SetAction(mbk->YesNoQuestion,0xFC0,Cancel2DelShortcut);
